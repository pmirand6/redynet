...
function QueOtros($fcurriculum, $fherramienta, $fidioma, $fconn){
	...
	$rqn=mysql_query($sqlqn,$fconn);
	if($rsqn = mysql_fetch_array($rqn)){
		$fdev = $rsqn["otros"];
	}
return($fdev);
}
...
function GetRama($fcategoria, $fconn){
	...
	while ($fvece[$fposvec]==0 && ($fposvec <= (count($fvece)-1))){
		...
		$rgr = mysql_query($sqlgr, $fconn);
		while($rsgr = mysql_fetch_array($rgr)){ 
			$fposvecadd++;
...
function CuantosAnuncios($fcategoria, $fconn){
	...
	$rca = mysql_query($fsqca, $fconn);
	if($rsca = mysql_fetch_array($rca)){ 
...
function FirstCategory($fcontenido, $fconn){
	...
	while ($cond=="f"){
		...
		$rur = mysql_query($sqlur, $fconn);
		if($rsur = mysql_fetch_array($rur)){ 
			$fcodigo = $rsur["codigo"];
			$fpadre = $rsur["padre"];
		} else {
			$cond="t";
...
function GrabarLog($funcsql, $fusuario, $fconn){
	...
	mysql_query($cons,$fconn) or die(mysql_error());
}
...
function MostrarHTML($fhtml, $fconn){
	...
	for($j=0;$j<$vcantimg;$j++){
		...
		$rimg = mysql_query($sqlimg, $fconn);
		if($rsimg = mysql_fetch_array($rimg)){ 
...
function getcodigounico($fconn) {
	...
	do {
		...
		$frcont = mysql_query($fsql, $fconn);
		if($frscont = mysql_fetch_array($frcont)){
	    	$festa = 0;
		} else {
			$festa = 1;
		}
